  ...
  shell(`
    <form id="tournament-form" class="stack">
      ...
      <section class="form-card">
        ...
        <p class="hint">同点の場合、この登録順で上位になります。</p>
      </section>
      <section class="form-card">
...
function renderTournament() {
  ...
  const history = [...tournament.games]
    ...
    .map((game, reverseIndex) => {
      ...
      const winner = game.results.find((result) => result.rank === 1);
      return `
        <button class="history-row" data-nav="game/${tournament.id}/${game.id}">
          ...
          <span><small>1着</small><strong>${escapeHtml(tournament.players[winner.index].name)}</strong></span>
          <b class="positive">${formatPt(winner.finalPt)}</b>
          ${icon("chevron")}
        </button>`;
    })
    .join("");
...
function updateLiveResult() {
  ...
  target.innerHTML = `
    ...
    <div class="preview-table">
      ${sorted.map((result) => `
        <div>
          <span class="rank-badge rank-${result.rank}">${result.rank}</span>
    ...
    <section class="result-list">
      ${sorted.map((result) => `
        <article class="result-card rank-card-${result.rank}">
          <div class="result-rank"><strong>${result.rank}</strong><span>着</span></div>
